...
- `첨부파일` (선택): 구글드라이브 공유 링크를 넣으면 상세 페이지에 "첨부파일" 다운로드 버튼이 나타납니다. 비워두면 버튼이 표시되지 않습니다. 다운로드 시 비밀번호가 필요하며, Vercel 환경변수 `MINUTES_PASSWORD`로 설정한 값과 같습니다 (임원진 회의록 다운로드와 동일한 비밀번호를 씁니다).
